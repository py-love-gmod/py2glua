...
class BuildCache:
    ...
    def validate(self, project_root: Path, file_pass: list[str]) -> None:
        ...
        valid_hashes = {_hash_path(p) for p in self.valid_paths}

        for f in self._cache_dir.glob("*.pkl"):
            if f.stem not in valid_hashes:
                f.unlink(missing_ok=True)

        for f in self._cache_dir.glob("*.dep"):
            if f.stem not in valid_hashes:
                f.unlink(missing_ok=True)

    def load_ir(self, path: Path) -> PyIRFile | None:
    ...
    def load_deps(self, path: Path) -> list[Path] | None:
        assert self._cache_dir is not None
        dep_file = self._cache_dir / f"{_hash_path(path)}.dep"
    ...
    def store(
        ...
        if deps is not None:
            dep_file = self._cache_dir / f"{_hash_path(path)}.dep"
            with dep_file.open("w", encoding="utf-8") as f:
                json.dump([str(p.resolve()) for p in deps], f)
