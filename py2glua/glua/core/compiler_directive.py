...
class CompilerDirective:
    ...
    def gmod_prototype(fold: str) -> Callable:
        """DLS макрос для классво. Позволяет создавать более простыми конструкциями однотипные айтемы и структуры (к примеру SWEP или ENT)

        Вешается на класс с указанием папки билда `fold`

        Обязазательно реализовывать поле `uid` для оверрайда айди предмета
    ...
    def register_arg(
        ...
        reg_type:
            ...
            material = Для билда материалов/текстур - WIP (На текущий момент это бесполезные параметр)
            model = Для билда моделей - WIP (На текущий момент это бесполезные параметр)
        """
